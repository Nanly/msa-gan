...
from mysdir.trains2.plane2.sub_input.dataset import config, Dataset
import numpy as np
...
def run(config, dataset, model, gpu):
    ...
    ckpt = tf.train.get_checkpoint_state(config.model_dir.replace('train_log_saveBN', 'train_log'))
